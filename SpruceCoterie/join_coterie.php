...
<body style="background-image: url('images/a1.jpg'); background-repeat: no-repeat; background-attachment: fixed;" align=center>
...
<div class="container" style="margin-top:50px">
			...
			<?php 
				$sql1 = "SELECT * FROM coterie WHERE 1";
				...
				{	
					...
					$x = $row["id"];
					echo '<div class="fakeim" style="padding-top:8px; padding-bottom:10px; padding-left:15px; padding-right:8px; border-radius:5px;" ><h4><b>Name: </b>'.$row["name"].'</h3></br><h5><b>Description:</b> '.$row["description"].'</h5></br><h6><b>Interests</b>: '.ucwords($row["inte"]).'</h6>
						 <form id="formid'.$x.'" method="GET" > <label style:"margin-left:50px;"> Desire </label> 
							<input type="submit" class="btn btn-info" id="sub'.$x.'" name="subb'.$x.'" value="JOIN" />
						  </form></div>';
					...
					if(isset($_GET['subb'.$x])){
						//$addc = $_GET['selc'.$x];
						$sql2 = "INSERT into cmem (cid, user) VALUES ( ".$x." , '".$_SESSION['user']."')";
						if(mysqli_query($con, $sql2)){
							mysqli_close($con);
							$_SESSION['cid'] = $x;
