...
<div class="container" style="margin-top:50px">
	<div class="row">
		<div class="col-sm-3">
			<div class="btn-group">
			  <button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
				Your coterie...  &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp 
			  </button>
			  <div class="dropdown-menu">
				<?php 
					$sql1 = "SELECT * FROM cmem WHERE user = '".$_SESSION['user']."'";
					$con = new mysqli("localhost","root","","sprucecoteriedb");
					if (!$con)
					{
						die('Could not connect: ' . mysql_error());
					...
					while($row = mysqli_fetch_array($ret)){
						$cid = $row['cid'];
						$sql2 = "SELECT * FROM coterie WHERE id = ".$cid."";
						$ret1 = mysqli_query($con, $sql2);
						while($roo = mysqli_fetch_array($ret1)){
							echo '<a class="dropdown-item" href="coterie.php">'.$roo["name"].'</a>';
						}
					}
				?>
			...
			<?php
				...
				if(!mysqli_query($con, $sql1)){}
					...
					echo '<h2 class="nav-link active">Lonely are we?</h2>
							<ul class="nav nav-pills flex-column">
								<li class="nav-item">
		...
		<div class="col-sm-6">
			...
			<?php 
				$sql1 = "SELECT * FROM events WHERE 1";
				...
				while($row = mysqli_fetch_array($result))
				{
					...
					if(isset($_GET['selc'.$x])){
						$addc = $_GET['selc'.$x];
						$sql2 = "UPDATE events SET count = count + ".$addc." WHERE id=".$x."";
						if(mysqli_query($con, $sql2)){
						}
						else{
		...
		<div class="col-sm-2">
			<ul class="nav nav-pills flex-column">
				<li class="nav-item">
					<a href="createevent.php" class="nav-link active" >Event +</a>
				</li> </br>
				<li class="nav-item">
					<a href="changedp.php" class="nav-link active" >Change Display Picture</a>
				</li> </br>
				<li class="nav-item">
					<a href="profileview.php" class="nav-link active" >View Profile</a>
				</li>
